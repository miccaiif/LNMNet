...
parser.add_argument('--model_path', type=str, default='./final_model/final_model_scale3/',
                    help='root of the test dataset')
# For dataloader
...
def funing(path, j):
    ...
    return test_error, confidence_bag, label_bag, predict_bag, j


if __name__ == "__main__":
    judgement = []
    judgement_path = './NPscale3_confidence_perbag.csv'
    ...
    for idx, filename in enumerate(os.listdir(args.model_path)):
        ...
        for seed in tqdm(range(0, 20)):
            ...
            since = time.time()
            test_error, confidence_bag, label_bag, predict_bag, j = funing(path=search_model_path, j=j)
    ...
    jug = np.array(judgement)
    bag_confidence = jug.squeeze()
    max_aggre_slide = np.amax(bag_confidence, axis=0)
    mean_aggre_slide = np.mean(bag_confidence, axis=0)
    #
    fpr, tpr, thersholds = roc_curve(y_label, max_aggre_slide)
    roc_auc = auc(fpr, tpr)
    # fpr, tpr, thersholds = roc_curve(y_label, mean_aggre_slide)
    # roc_auc = auc(fpr, tpr)
    drawing_ROC(fpr, tpr, roc_auc)

    np.savetxt(judgement_path, jug.squeeze(), delimiter=',')
    # np.savetxt(result_auc_save_path, result_auc, delimiter=',')
    print(name)
